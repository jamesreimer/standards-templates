...
Agent-oriented repositories or local agent guidance may consume and operationalize actor-neutral standards without redefining their underlying governance. This file is repository-local agent guidance, not a reusable standard template.
...
- When required local or hosted validation applies to a candidate, the candidate is not complete, review-ready, or eligible for handoff to the next gated action until every required check has reached a final successful result. An agent MUST NOT close implementation, report completion, or return a candidate as ready while any required validation is pending, queued, in progress, cancelled, skipped unexpectedly, or otherwise unresolved. The agent remains responsible for the validation gate until all required checks succeed or a check reaches a final non-success state. On non-success, treat the candidate as incomplete and either correct the failure within existing authority or return the work as blocked or requiring review. A pending check is not successful validation, and an open pull request is not evidence that the reviewable-candidate boundary has been reached.
