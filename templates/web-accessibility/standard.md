...
**WEB-A11Y-013 — No unsupported completion claims.** A conformance conclusion MUST be supported for its entire declared scope. Missing, stale, inapplicable, unperformed, or unresolved evidence MUST NOT be converted into a passing result. Known failures MUST remain visible even if other coverage is incomplete. Claims about approval, evaluation completion, or effectiveness MUST be true at the time made; text intended to survive publication MUST use stable or explicitly event-conditioned wording when the relevant act has not yet occurred. Wording does not create the underlying act or authority.
