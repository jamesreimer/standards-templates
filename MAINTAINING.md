...
When creating or materially revising a template, assess whether adopters need explicit workflow-role guidance. When appropriate, include a `Workflow integration` section in the template `README.md` as informative adoption/integration material, not in `standard.md`. Identify when the adopted standard should be consulted; adaptable guidance for a contributor entry point such as `AGENTS.md` may be included. Make clear that the adopted local artifact governs after adoption. Do not require a particular tool or instruction filename, introduce an adoption dependency, or duplicate the standard's normative requirements. This section is not universally required.
